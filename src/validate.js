import { ERROR_HEADER, PURCHASE_UNIT } from "./const.js";

export const validateEmptyInput = (input, field) => {
  if (!input || input === "") {
    throw new Error(`${ERROR_HEADER} ${field}의 값이 존재하지 않습니다.`);
  }
};
...
export const validateThousandUnit = (input) => {
  if (Number(input) % PURCHASE_UNIT !== 0) {
    throw new Error(
      `${ERROR_HEADER} 구입 금액의 입력값은 ${PURCHASE_UNIT}단위 숫자로 입력해 주세요.`
    );
  }
};
...
export const validateNumbersLength = (numbers, field) => {
  if (numbers.length !== 6) {
    throw new Error(`${ERROR_HEADER} ${field}는 6개여야 합니다.`);
  }
};
...
export const validateDuplicateNumbers = (numbers, field) => {
  ...
  if (uniqueNumbers.size !== numbers.length) {
    throw new Error(`${ERROR_HEADER} ${field}에 중복된 숫자가 있습니다.`);
  }
};
